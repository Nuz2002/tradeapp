...
useEffect(() => {
  ...
  if (period === "monthly") {
    ...
    // Create a single bucket for today
    const todayKey = now.toISOString().slice(0, 10);
    addBucketIfMissing(todayKey);
    trades.forEach((t) => {
      const dt = new Date(t.timestamp);
      const tradeDateKey = dt.toISOString().slice(0, 10);
      if (tradeDateKey === todayKey) {
        buckets[todayKey].profit += parseNumber(t.profit || 0);
        buckets[todayKey].tradeValue += parseNumber(t.tradeValue || 0);
      }
    });
  } else {
  ...
  if (period === "today") {
    // For today, we now have a single bucket
    const bucket = distributed[0] || { key: now.toISOString().slice(0, 10), profit: 0 };
    chart = [{
      label: new Date(bucket.key).toLocaleDateString("en-US", { 
        month: "long", 
        day: "numeric" 
      }),
